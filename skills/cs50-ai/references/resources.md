# CS50 AI - Additional Resources

## Official Course Materials

- **Course Website**: https://cs50.harvard.edu/ai/
- **Lecture Videos**: https://cs50.harvard.edu/ai/2024/
- **Shorts**: Supplementary videos on specific topics
- **Problem Sets**: Hands-on projects with autograder
- **Source Code**: https://cdn.cs50.net/ai/

## Lecture Notes & Summaries

### Week 1: Search
- [MIT OCW 6.034 - Search](https://ocw.mit.edu/courses/6-034-artificial-intelligence-fall-2010/)
- [Stanford CS221 - Search](https://cs221.stanford.edu/)
- [AIMA Chapter 3-4](http://aima.cs.berkeley.edu/)

### Week 2: Knowledge
- [AIMA Chapter 7-9](http://aima.cs.berkeley.edu/)
- [Stanford CS221 - Logic](https://cs221.stanford.edu/)

### Week 3: Uncertainty
- [AIMA Chapter 12-14](http://aima.cs.berkeley.edu/)
- [Seeing Theory - Probability](https://seeing-theory.brown.edu/)

### Week 4: Optimization
- [AIMA Chapter 6](http://aima.cs.berkeley.edu/)
- [GeeksforGeeks - CSP](https://www.geeksforgeeks.org/constraint-satisfaction-problems-csp/)

### Week 5: Machine Learning
- [Google ML Crash Course](https://developers.google.com/machine-learning/crash-course)
- [Scikit-learn Documentation](https://scikit-learn.org/stable/)
- [StatQuest YouTube](https://www.youtube.com/c/joshstarmer)

### Week 6: Neural Networks
- [3Blue1Brown - Neural Networks](https://www.youtube.com/playlist?list=PLZHQObOWTQDNU6R1_67000Dx_ZCJB-3pi)
- [Neural Networks and Deep Learning](http://neuralnetworksanddeeplearning.com/)
- [CS231n - CNNs](http://cs231n.stanford.edu/)

### Week 7: Language
- [Stanford CS224n - NLP](https://web.stanford.edu/class/cs224n/)
- [Speech and Language Processing](https://web.stanford.edu/~jurafsky/slp3/)

## Textbooks

### Primary
- **Artificial Intelligence: A Modern Approach** (AIMA)
  - Authors: Stuart Russell, Peter Norvig
  - Website: http://aima.cs.berkeley.edu/
  - Free online chapters available

### Supplementary
- **Deep Learning** (Goodfellow et al.)
  - Free online: https://www.deeplearningbook.org/
- **Pattern Recognition and Machine Learning** (Bishop)
- **An Introduction to Statistical Learning** (ISLR)
  - Free PDF: https://www.statlearning.com/

## Video Lectures

### Complete Courses
- [MIT 6.034 - Artificial Intelligence](https://www.youtube.com/playlist?list=PLUl4u3cNGP63gFHB6xb-kVBiQHYe_4hSi)
- [Stanford CS221 - AI Principles](https://www.youtube.com/playlist?list=PLoROMvodv4rO1NB9TD4iUZ3qghGEGtqNX)
- [Stanford CS229 - Machine Learning](https://www.youtube.com/playlist?list=PLoROMvodv4rMiGQp3WXShtMGgzqpfVfbU)
- [Stanford CS231n - CNNs](https://www.youtube.com/playlist?list=PL3FW7Lu3i5JvHM8ljYj-zLfQRF3EO8sYv)
- [Stanford CS224n - NLP](https://www.youtube.com/playlist?list=PLoROMvodv4rOSH4v6133s9LFPRHjEmbmJ)

### Topic-Specific
- [3Blue1Brown - Neural Networks](https://www.youtube.com/playlist?list=PLZHQObOWTQDNU6R1_67000Dx_ZCJB-3pi)
- [3Blue1Brown - Linear Algebra](https://www.youtube.com/playlist?list=PLZHQObOWTQDPD3MizzM2xVFitgF8hE_ab)
- [StatQuest - ML Algorithms](https://www.youtube.com/c/joshstarmer)
- [Yann LeCun - Deep Learning](https://www.youtube.com/watch?v=FW5g89fJgvU)

## Interactive Tools

### Visualization
- [TensorFlow Playground](https://playground.tensorflow.org/)
- [ConvNetJS](https://cs.stanford.edu/people/karpathy/convnetjs/)
- [Netron - Neural Network Visualizer](https://netron.app/)
- [GraphViz Online](https://dreampuf.github.io/GraphvizOnline/)

### Practice
- [Kaggle](https://www.kaggle.com/) - Datasets and competitions
- [Google Colab](https://colab.research.google.com/) - Free GPU notebooks
- [LeetCode](https://leetcode.com/) - Algorithm practice
- [HackerRank](https://www.hackerrank.com/) - Python and ML challenges

### Data
- [UCI ML Repository](https://archive.ics.uci.edu/ml/index.php)
- [Kaggle Datasets](https://www.kaggle.com/datasets)
- [Google Dataset Search](https://datasetsearch.research.google.com/)
- [Papers With Code](https://paperswithcode.com/)

## GitHub Repositories

### Course Materials
- [cs50/ai](https://cdn.cs50.net/ai/) - Official CS50 AI code
- [aimacode/aima-python](https://github.com/aimacode/aima-python) - AIMA implementations

### Project Examples
- [microsoft/ai](https://github.com/microsoft/ai) - Microsoft AI samples
- [tensorflow/examples](https://github.com/tensorflow/examples)
- [pytorch/tutorials](https://github.com/pytorch/tutorials)

### Datasets & Pre-trained Models
- [huggingface/transformers](https://github.com/huggingface/transformers)
- [keras-team/keras-applications](https://github.com/keras-team/keras-applications)

## Blogs & Articles

### AI Fundamentals
- [Towards Data Science](https://towardsdatascience.com/)
- [Machine Learning Mastery](https://machinelearningmastery.com/)
- [Distill.pub](https://distill.pub/) - Visual explanations
- [Colah's Blog](https://colah.github.io/) - Neural network visualizations

### Research
- [ArXiv.org](https://arxiv.org/) - Preprints
- [Papers With Code](https://paperswithcode.com/) - Papers with implementations
- [Google AI Blog](https://ai.googleblog.com/)
- [OpenAI Blog](https://openai.com/blog/)

## Communities

### Forums
- [CS50 Discord](https://discord.gg/cs50) - Course community
- [r/MachineLearning](https://www.reddit.com/r/MachineLearning/)
- [r/learnmachinelearning](https://www.reddit.com/r/learnmachinelearning/)
- [Stack Overflow](https://stackoverflow.com/) - Technical questions

### Study Groups
- [CS50 Study Groups](https://cs50.harvard.edu/ai/2024/)
- [ML Study Group](https://www.meetup.com/topics/machine-learning/)

## Tools & Libraries

### Python Libraries
```bash
# Core ML
pip install numpy pandas scikit-learn

# Deep Learning
pip install tensorflow torch torchvision

# NLP
pip install nltk spacy transformers

# Visualization
pip install matplotlib seaborn plotly

# Utilities
pip install jupyter notebook tqdm
```

### Development Environment
- [Jupyter Notebook](https://jupyter.org/)
- [Google Colab](https://colab.research.google.com/) - Free GPU
- [VS Code](https://code.visualstudio.com/) with Python extension
- [PyCharm](https://www.jetbrains.com/pycharm/) - Professional IDE

## Tips for Success

1. **Start with the official materials** - They're well-designed
2. **Watch lectures at 1.5x speed** - Save time, rewatch difficult parts
3. **Code along** - Don't just watch, implement
4. **Use office hours** - CS50 staff are incredibly helpful
5. **Join the community** - Discord, forums, study groups
6. **Build projects** - Apply what you learn to personal projects
7. **Read papers** - Start with survey papers in areas of interest
8. **Stay updated** - AI moves fast, follow key researchers
